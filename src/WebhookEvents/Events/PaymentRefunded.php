...
/**
 * Class for payment refunded webhook events
 */
class PaymentRefunded extends BaseEvent
{
	/**
	 * @var string The event type
	 */
	protected string $type = 'jovvie.payment.refunded';
	...
	/**
	 * Get the refund amount associated with the payment.
	 *
	 * @return int Refund amount
	 * @throws WebhookError If invoice refund is missing in the payload
	 */
	public function getInvoiceRefund(): int
	{
		if (!isset($this->payload['invoice']['refund'])) {
			throw new WebhookError('Missing invoice.refund in payload');
		}
		return $this->payload['invoice']['refund'];
	}
